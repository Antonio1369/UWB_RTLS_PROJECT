...
void setup() {
  ...
  DW1000Ng::setReceiveFrameWaitTimeoutPeriod(7000);
...
void loop() {
  if (DW1000NgRTLS::receiveFrame()) {
    ...
    if (recv_data[0] == BLINK) {
      DW1000NgRTLS::transmitRangingInitiation(&recv_data[2], NULL);
      DW1000NgRTLS::waitForTransmission();
      RangeAcceptResult result = DW1000NgRTLS::anchorRangeAccept(NextActivity::RANGING_CONFIRM, next_anchor);

      if (!result.success) {
      Serial.println("Result Unsucs");
      return;
      }

      tag_flag = recv_data[2];
      String rangeString = "tag-";
      rangeString += recv_data[2];
      rangeString += " Range: ";
      rangeString += result.range;
      rangeString += " m";
      rangeString += "\t RX power: ";
      //Serial.println("Antes getreceiverpower");
      rangeString += DW1000Ng::getReceivePower();
      //Serial.println("Despues getreceiverpower");
      rangeString += " dBm";
      Serial.println(rangeString);

    }
    //CAMBIAR LA LOGICA PARA QUE SEA QUE LEE 0X60 QUE ES UNA ANTENA PERO TAMBIEN HACER QUE EL ID DE LA ANTENA SEA IGUAL A LOS IDS UNICOS DE ESTAS DOS ANTENAS
    else if (recv_data[9] == 0x60) {
      double range = static_cast<double>(DW1000NgUtils::bytesAsValue(&recv_data[10], 2) / 1000.0);
      String rangeReportString = "";

      rangeReportString = "tag-";
      rangeReportString += tag_flag;
      ...
      if (received_B == false && recv_data[7] == anchor_b[0] && recv_data[8] == anchor_b[1]) {
        ...
        String positioning = "Found position(tag-";
        Serial.println(String(tag_flag));
        positioning += tag_flag;
        positioning += ") - x: ";
        positioning += x;
        positioning += " y: ";
        positioning += y;
